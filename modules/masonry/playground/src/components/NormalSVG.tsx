...
const NormalSVG: React.FC = () => {
  ...
  const findConnectedBricks = (startId: number): number[] => {
    ...
    while (true) {
      ...
      const nextBrick = bricks.find(
        (b) => b.id !== currentId && isConnected(currentBrick!, b),
      );
  ...
  return (
    ...
    >
      {bricks.map((brick) => (
        ...
        >
          <path d={renderBrickPath(brick.type)} fill={brick.color} stroke="black" />
        </g>
      ))}
    </svg>
  );
